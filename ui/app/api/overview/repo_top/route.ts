...
const repoLabel = "COALESCE(NULLIF(trim(s.git_repository_url), ''), '<unknown>')";
...
export const GET = (request: NextRequest) => {
  try {
    ...
    const rows = db
      .prepare(
        `SELECT ${repoLabel} as label, SUM(e.total_tokens) as total_tokens
        FROM events e
    ...
    if (rows.length) {
      const otherRow = db
        .prepare(
          ...
          LEFT JOIN sessions s ON s.session_id = e.session_id
          ${eventsWhere.sql}
          AND ${repoLabel} NOT IN (${rows.map(() => "?").join(",")})`
        )
    ...
    return errorResponse(
      error instanceof Error ? error.message : "Failed to load repo top",
      500
    );
  }
};
