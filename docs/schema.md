...
The tracker stores append-only events in SQLite. Each row represents a usage line or a `/status` snapshot. No prompt or response content is stored.
...
- `event_type` (TEXT): `usage_line` or `status_snapshot`.
- `total_tokens` (INTEGER): total tokens reported by Codex.
...
- `source` (TEXT): capture source (currently `stdout`).
